...
@section('main_content')
    ...
    <div class="col-sm-3 ps-5 my-4">
        <a href="{{ route('sucursal.create') }}" class="btn btn-success">
            <i class="bi bi-plus-square"></i> Nueva Sucursal</a>
    </div>
    <!-- FIN BOTON NUEVA SUCURSAL -->
            ...
            <tbody class="text-center">
                @foreach ($sucursal as $sucursal)
                    <tr>
                        ...
                        <td>{{ $sucursal->calle . ', ' . $sucursal->codigoPostal . ', ' . $sucursal->colonia . ', ' . $sucursal->municipio . ', ' . $sucursal->entidadFederativa->nombreEstado }}
                        </td>
                        <td>{{ $sucursal->referencias }}</td>
                        <td>
                            <a href="{{ route('sucursal.edit', $sucursal->id) }}" class="btn btn-success btn-sm"
                                data-bs-toggle="tooltip" data-bs-title="Editar datos">
                                <i class="bi bi-pencil-square"></i>
                            </a>
                            <form action="{{ route('sucursal.destroy', $sucursal->id) }}" method="POST"
                                style="display: inline;" id="formEliminar{{ $sucursal->id }}">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="btn btn-danger btn-sm"
                                    onclick="confirmarEliminacion('{{ $sucursal->id }}')" data-bs-toggle="tooltip"
    ...
    <script>
        ...
        function confirmarEliminacion(sucursalId) {
            ...
            var formulario = document.getElementById('formEliminar' + sucursalId);
                    ...
                    fetch(formulario.action, {
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                ...
                                // Elimina la fila de la tabla correspondiente a la sucursal eliminada
