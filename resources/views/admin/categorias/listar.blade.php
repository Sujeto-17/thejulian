...
@section('main_content')
    ...
    <div class="col-sm-3 ps-5 my-4">
        <a href="{{ route('categorias.create') }}" class="btn btn-success">
            <i class="bi bi-plus-square"></i> Nueva Categoría</a>
    </div>
    <!-- FIN BOTON NUEVA CATEGORIA -->
        ...
        <table class="table table-bordered table-striped" id="MiTabla">
            ...
            <tbody class="text-center">
                @foreach ($categorias as $categoria)
                    <tr>
                        <td>{{ $categoria->id }}</td>
                        <td>{{ $categoria->claveCategoria }}</td>
                        ...
                        <td>
                            ...
                            <form action="{{ route('categorias.destroy', $categoria->id) }}" method="POST"
                                style="display: inline;" id="formEliminar{{ $categoria->id }}">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="btn btn-danger btn-sm"
                                    onclick="confirmarEliminacion('{{ $categoria->id }}')" data-bs-toggle="tooltip"
                                    data-bs-title="Eliminar Categoria">
    ...
    <!-- FIN TABLAS CATEGORIA -->

    <!-- Script para confirmar eliminación de categoría -->
    <script>
        ...
        function confirmarEliminacion(categoriaId) {
            ...
            var formulario = document.getElementById('formEliminar' + categoriaId);
                    ...
                    fetch(formulario.action, {
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                ...
                                // Elimina la fila de la tabla correspondiente a la categoría eliminada
